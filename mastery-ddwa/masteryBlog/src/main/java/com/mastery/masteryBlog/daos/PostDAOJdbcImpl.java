...
public class PostDAOJdbcImpl implements PostDAO {
    ...
    private final String createPost = "Insert into Post(`username`, title, postDate, catagoryId, posting, isApproved, isStatic, imagePath) Values (?,?,?,?,?,?,?,?)";
    private final String getAllPosts = "Select Post.postId, Post.`username`, Post.title, Post.postDate, cat.catagoryId, cat.`description`, Post.posting, group_concat(t.tagId), group_concat(t.tagName), Post.isApproved, Post.isStatic, Post.imagePath From Catagory As cat"
                                     + " Inner Join Post On Post.catagoryId = cat.catagoryId"
                                     + " Left Join PostTags pt on Post.postId = pt.postId"
                                     + " Left Join Tags t on pt.tagId = t.tagId"
                                     + " group by post.postId";
    private final String getPostById = "Select Post.postId, Post.`username`, Post.title, Post.postDate, cat.catagoryId, cat.`description`, Post.posting, group_concat(t.tagId), group_concat(t.tagName), Post.isApproved, Post.isStatic, Post.imagePath From Catagory As cat"
                                     + " Inner Join Post On Post.catagoryId = cat.catagoryId"
                                     + " Left Join PostTags pt on Post.postId = pt.postId"
                                     + " Left Join Tags t on pt.tagId = t.tagId"
                                     + " Where Post.postId = ?"
                                     + " group by post.postId";
    private final String getPostByCategoryId = "Select Post.postId, Post.`username`, Post.title, Post.postDate, cat.catagoryId, cat.`description`, Post.posting, group_concat(t.tagId), group_concat(t.tagName), Post.isApproved, Post.isStatic, Post.imagePath From Catagory As cat"
            + " Inner Join Post On Post.catagoryId = cat.catagoryId"
            + " Left Join PostTags pt on Post.postId = pt.postId"
            + " Left Join Tags t on pt.tagId = t.tagId"
            + " Where cat.catagoryId = ?"
            + " group by post.postId";
    private final String getPostByTagName = "Select Post.postId, Post.`username`, Post.title, Post.postDate, cat.catagoryId, cat.`description`, Post.posting, group_concat(t.tagName), Post.isApproved, Post.isStatic, Post.imagePath From Catagory As cat "
    ...
    public Post addPost(Post post) {
        jdbc.update(createPost, post.getUsername(), post.getTitle(), post.getDateTime(), post.getCategory().getCatagoryId(), post.getPosting(), post.isApproved(), post.isIsStatic(), post.getImagePath());
